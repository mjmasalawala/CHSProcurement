...
- **WhatsApp — two real events are live in production:**
  ...
  - **OTP (`sendWhatsappOtp`) is built but currently dead code** — nothing in the app calls it (invite acceptance bypasses phone verification entirely today), and no Authentication-category OTP template has been submitted to the *production* WABA yet (only the Test WABA has one, `wisesoc_otp_code`). Not an active risk since nothing invokes it, but don't assume OTP-over-WhatsApp works in production without checking this first.
...
|---|---|---|---|---|
| auth.otp | any | W (E fallback) | Authentication | phone verification — **built (`sendWhatsappOtp`), but dead code today; no production Authentication template exists yet (Section 0)** |
